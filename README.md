...
1. Observer : 주체에 구독, 제거하여 주체의 상태변화를 감지하고자 하는 객체   
2. Subject : 상태 변화시 자신에게 등록된 관찰자들에게 변화됐음을 알리는 객체  

<img width="575" alt="옵저버 패턴" src="https://github.com/user-attachments/assets/3a1d6f87-7102-4d43-937d-20d397acecde">

사진에서는 Client가 Reciever에게 요청하지만, 실제로는 Invoker에게 요청한다.


장점  
1. 주체에 필요한 만큼 관찰자를 추가할 수 있다. 런타임에도 유동적으로 변동가능하다.
2. 옵저버 패턴의 주요 이점은 하나의 주체에 대해 다수의 관찰자 객체 간의 이벤트 처리를 할 수 있다는 점이다. 일대다 관계가 있는 객체 간의 이벤트 처리 시스템 구현 문제를 해결할 수 있다.

단점  
1. 관찰자들의 알림 순서를 보장받지 못한다. 둘 이상의 관찰자 객체가 하나의 주체 객체에 등록했을 때 두 객체가 특정 순서에 맞춰 작동해야 했을 때 기본 형태의 옵저버 패턴은 적당하지 않다.
2. 주체는 관찰자에 대한 강한 참조를 갖기 때문에 분리 삭제를 정확히 하지 않으면 객체를 쓰지 않음에도 GC가 회수 하지 않을 수 있다.
